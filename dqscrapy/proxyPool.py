...
class Proxy(object):
    def __init__(self, max_page=1):
        ...
        self.headers = {
            ...
            'Cookie': 'peuland_id=35fefe23fedc52da9283ac5ed131cbab;PHPSESSID=pkm7b65es5ojb8oerc7a9i0q31; peuland_md5=ca1f57155f5638ade3c28a900fbdbd55;w_h=800; w_w=1280; w_cd=24; w_a_h=773; w_a_w=1280; php_id=2006321314'
        }
    ...
    def _parse_proxy(self):
        ...
        while (i <= self.max_page):
            ...
            for line in data:
                ...
                levelType = base64.b64decode(line['level_type'])
                timeDownLoadSpeed = base64.b64decode(line['time_downloadspeed'])
                contryCode = base64.b64decode(line['country_code'])
                countryZw = base64.b64decode(line['country_zw'])
                statusCnc = base64.b64decode(line['status_cnc'])
                statusTnc = base64.b64decode(line['status_ctn'])
                self.proxies.append({proxy_type: ip + ':' + port,"levelType":levelType,"timeDownLoadSpeed":timeDownLoadSpeed,"contryCode":contryCode,
                                     "countryZw":countryZw,"statusCnc":statusCnc,"statusTnc":statusTnc})
            i = i + 1
...
if __name__ == '__main__':
    ins = Proxy()
    # print ins.get_proxy()
    proxyList = ins.get_proxy()
    if isinstance(proxyList,list):
        # for pList in proxyList:
            # for proxyType,proxyIp in pList.items():
            #     # ProxyModelDispatcher.saveProxyInfo(mProxyIp = proxyIp,mProxyType=proxyType)
            #     # print proxyType,proxyIp
            #       ProxyModelDispatcher.saveProxyDict()
       ProxyModelDispatcher.saveProxyDict(mProxyList=proxyList)
